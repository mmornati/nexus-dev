...
@click.argument("name", required=False)
@click.option(
    "--all", "-a", "test_all", is_flag=True, help="Test all configured servers, not just active profile"
)
def mcp_test_command(name: str | None, test_all: bool) -> None:
    ...
    if name:
        ...
        click.echo(f"Testing {len(servers_to_test)} servers in active profile '{mcp_config.active_profile}'...")
    ...
    async def _test_servers():
        ...
        for srv_name, srv_config in servers_to_test:
            ...
            if not srv_config.enabled:
                click.echo(f"  ⏭️  Skipped: Server is disabled in config.")
                click.echo()
                continue
            ...
            try:
                ...
                click.echo(f"  ✅ Success! Connection established.")
